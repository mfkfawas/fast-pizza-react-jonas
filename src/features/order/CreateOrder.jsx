...
function CreateOrder() {
  ...
  // if we wired in our routes properly, then we can access the data from the action function here.
...
export async function action({ request }) {
  // getting data from the form(react-router style)
  ...
  const errors = {};
  if (!isValidPhone(order.phone)) errors.phone = 'Mobile Number Invalid';

  if (Object.keys(errors).length > 0) return errors;

  // const newOrder = await createOrder(order);

  // we cannot call hooks inside these normal fns
